...
# risk_fun <- function(theta) {
 ...
 #  loss <- (theta - trueCATE)^2
  ...
  lrnr_names <- colnames(fit_npcausalML$full_predictions)
  data <- data.table(CATE = unlist(fit_npcausalML$cv_predictions),lrnr = rep(lrnr_names, n) , Method = "EP")
    data <- rbind(data,data.table(CATE = unlist(CATEonestepbench_cv),lrnr = rep(names(CATEonestepbench_cv), n) , Method = "DR"))

  data$sieve <- str_match(data$lrnr, "fourier_basis_([0-9])")[,2]
  data$depth <- str_match(data$lrnr, "ranger_500_TRUE_none_1_30_([0-9])")[,2]
   risks<-  data[,risk_fun(CATE), by = c("depth", "sieve", "Method")]
 sieve_dim <-  risks[, sieve[which.min(V1)], by = c("depth", "Method" )]
sieve_dim_EP <- sieve_dim[sieve_dim$Method=="EP"]

keep <- sapply(1:nrow(sieve_dim_EP), function(i){
  which(str_detect( lrnr_names, paste0("fourier_basis_", sieve_dim_EP[i,3]))  &  str_detect(lrnr_names, paste0("ranger_500_TRUE_none_1_30_", sieve_dim_EP[i,1])))
})


EP_best_by_depth <- fit_npcausalML$full_predictions[, keep, with = F]
  
cv_best_sieve <-  risks[Method == "EP", sieve[which.min(V1)], , by = c( "Method" )]
cv_best_depth <-  risks[Method == "EP", depth[which.min(V1)], , by = c( "Method" )]

keep <- which(str_detect( lrnr_names, paste0("fourier_basis_", cv_best_sieve[[2]]))  &  str_detect(lrnr_names, paste0("ranger_500_TRUE_none_1_30_", cv_best_depth[[2]])))
EP_cv <-    fit_npcausalML$full_predictions[, keep, with = F][[1]]


 cv_best_depth <-  risks[Method == "DR", depth[which.min(V1)], , by = c( "Method" )]
keep <- which(    str_detect(colnames(CATEonestepbench), paste0("ranger_500_TRUE_none_1_30_", cv_best_depth[[2]])))

DR_cv <-CATEonestepbench[,keep, with = F][[1]]
 ...
 data <- data.table(CATE = unlist(fit_npcausalML$full_predictions), CATE_cv =unlist(fit_npcausalML$cv_predictions),  lrnr = rep(lrnr_names, each = n) , Method = "EP")
  ...
  data$best_sieve[data$Method=="DR"] <- which(data$Method == "DR")
