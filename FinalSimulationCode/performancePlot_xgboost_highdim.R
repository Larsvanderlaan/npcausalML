...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      try({load(paste0("mainSimResults/simsCATE", hard, pos,  "n", n, "_xgboost_highDim"))
     ...
     # dt$degree <- as.numeric(stringr::str_match(dt$lrnr_full, "fourier_basis_([0-9]+)")[,2])
      ...
      tmp <- data.table(risks_oracle = onestepbenchoracle, lrnr_full = names(onestepbench), degree = "Oracle ne-step")
      dt <- rbind(dt, tmp, fill = T)
    ...
    ggsave(paste0("mainSimResults/performancePlot_CATE_HighDim_ranger", "pos=",pos, "hard=",hard, ".pdf"), width = 8, height = 7)
    ...
    max_depth[is.na(max_depth)] <- "cv"
    dt_tmp$lrnr <- paste0("ranger (", "max_depth=", max_depth,")")
    dt_tmp <- dt_tmp[max_depth %in% c( "9", "cv"),]
    dt_tmp$lrnr <- factor(dt_tmp$lrnr, levels = paste0("ranger (", "max_depth=", c("5", "7", "9", "11", "cv"),")"))
    ...
    dt <- rbind( dt_xg,dt_rf)
    dt <- dt[dt$type %in% c("Substitution", "One-step", "Sieve-Plugin", "Causal-Forest"),]
    ...
    ggsave(paste0("mainSimResults/FellowshipperformancePlot_CATE_HighDim_tree", "pos=",pos, "hard=",hard, "mainpaper.pdf"), width = 8, height = 7)
