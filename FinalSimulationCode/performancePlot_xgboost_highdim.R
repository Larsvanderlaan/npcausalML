  ...
  for(hard in hard_list) {
    try({
      sims_list <- lapply(ns, function(n) {
        try({load(paste0("mainSimResults2/mainSimResults2/simsCATE", hard, pos,  "n", n, "_xgboost_highDim"))
      ...
      for(lrnr in unique(dt$lrnr)) {
        ...
        ggsave(paste0("mainSimResults/plots/performancePlot_CATE_highDim_tree", "pos=",pos, "hard=",hard, "_", lrnr,".pdf"), width = 4, height = 4)
      }
