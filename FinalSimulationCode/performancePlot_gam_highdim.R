...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      try({
      load(paste0("mainSimResults/mainSimResults/simsCATE", hard, pos,  "n", n, "_gam_highDim"))
    ...
    ggsave(paste0("mainSimResults/performancePlot_GAM_CATE_highDim_", "pos=",pos, "hard=",hard, ".pdf"), width = 8, height = 7)
    ...
    dt_tmp <- dt_tmp[keep,]
    dt_tmp <- dt_tmp[!(dt_tmp$type == "Substitution"),]
    dt_tmp[(dt_tmp$type == "Sieve-Plugin"),"type"] <- "Sieve-plugin (*)"
    ...
    plt
    ggsave(paste0("mainSimResults/performancePlot_GAM_CATE_highDim_", "pos=",pos, "hard=",hard, "_mainpaper.pdf"), width = 8, height = 7)

    dt <- rbindlist(sims_list)
    dt <- dt[dt$type %in% c("Substitution", "One-step", "Sieve-Plugin"),]
    dt[dt$type %in% c(  "Substitution" ), "type"] <- "Substitution (T-Learner)"
    dt[dt$type %in% c(  "One-step" ), "type"] <- "DR-Learner"
    dt[dt$type %in% c( "Sieve-Plugin"),"type"] <- "plug-in DR-Learner(*)"
    ...
    dt$lrnr[grep("GAM",dt$lrnr)][is.na(s)] <- paste0("GAM (s=cv)")
    dt_tmp <- dt
    keep <- dt_tmp$lrnr %in% c("MARS (earth)",   "GAM (s=1)", "GAM (s=3)", "GAM (s=5)", "GAM (s=cv)")
    dt_tmp <- dt_tmp[keep,]
    dt_tmp <- dt_tmp[!(dt_tmp$type == "Substitution"),]
    dt_tmp[(dt_tmp$type == "Sieve-Plugin"),"type"] <- "Sieve-plugin (*)"
    ...
    plt <- plt + theme(legend.justification = c(0.9, 0), legend.position = c(0.95, 0.23))
    plt
    ggsave(paste0("mainSimResults/FellowshipperformancePlot_GAM_CATE_highDim_", "pos=",pos, "hard=",hard, "_mainpaper.pdf"), width = 8, height = 7)
