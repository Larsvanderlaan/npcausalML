...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      load(paste0("mainSimResults/simsCATE", hard, pos,  "n", n, "_xgboost_highDim"))
    ...
    ggsave(paste0("mainSimResults/performancePlotTrees_oracleSieve_xgboost", "pos=",pos, "hard=",hard, ".pdf"), width = 8, height = 7)
    ...
    ggsave(paste0("mainSimResults/performancePlotTrees_ranger", "pos=",pos, "hard=",hard, ".pdf"), width = 8, height = 7)
