...
fit_npcausalML <- npcausalML(list(Lrnr_glm$new()),
  ...
  set.seed(10030)
fit_npRR <- npRR::npRR(Lrnr_glm$new(), W, A, Y, V = W, weights, Vpred = V, sl3_Learner_EYAW = sl3_Learner_EYAW, sl3_Learner_pA1W = sl3_Learner_pA1W, list_of_sieves = list_of_sieves, outcome_type = "continuous")
```

```{r}
data.table(npcausalML::predict(fit_npcausalML, W))
data.table(npcausalML::predict(fit_npcausalML, W),
npRR:::predict(fit_npRR, W))
```
