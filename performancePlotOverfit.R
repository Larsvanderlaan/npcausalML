...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      load(paste0("mainSimResults/simsCATE", hard, pos,  "n", n, "_overfitting"))
    ...
    if(use_oracle_sieve){
      ggsave(paste0("mainSimResults/performancePlotRandomizedTrees_oracleSieve", "pos=",pos, "hard=",hard, "_overfitting.pdf"))
    } else {
      ggsave(paste0("mainSimResults/performancePlotRandomizedTrees", "pos=",pos, "hard=",hard, "_overfitting.pdf"))
    }
