...
onesim <- function(n) {
  ...
  lrnr_xg <- list(    Lrnr_xgboost$new(max_depth = 1, verbosity = 0, nrounds = 10),    Lrnr_xgboost$new(max_depth = 2, verbosity = 0, nrounds = 10), Lrnr_xgboost$new(max_depth = 3, verbosity = 0, nrounds = 10), Lrnr_xgboost$new(max_depth = 4, verbosity = 0, nrounds = 10),   Lrnr_xgboost$new(max_depth = 5, verbosity = 0, nrounds = 10),   Lrnr_xgboost$new(max_depth = 6, verbosity = 0, nrounds = 10) )
  ...
  rf_keep <- tmp[1:16,cvrisksDR[which.min(risk)], by = degree]$V1
  xg_keep <- tmp[-(1:16),cvrisksDR[which.min(risk)], by = degree]$V1
  names(xg_keep) <- paste0("Lrnr_xgboost_cv", "_fourier.basis_", 0:4, "_plugin")
  names(rf_keep) <- paste0("Lrnr_rf_cv", "_fourier.basis_", 0:4, "_plugin")
  cvrisksDR <- c(cvrisksDR, xg_keep, rf_keep)

  rf_keep <- tmp[1:16,cvrisksDRoracle[which.min(risk)], by = degree]$V1
  xg_keep <- tmp[-(1:16),cvrisksDRoracle[which.min(risk)], by = degree]$V1
  names(xg_keep) <- paste0("Lrnr_xgboost_cv", "_fourier.basis_", 0:4, "_plugin")
  names(rf_keep) <- paste0("Lrnr_rf_cv", "_fourier.basis_", 0:4, "_plugin")
  ...
  CATEonestepbench <- DR_learner(CATE_library, data.frame(W1=W1), A, Y, EY1W_est, EY0W_est, pA1W_est, NULL, NULL)
...
for(hard in hard_list) {
  for(pos in pos_list) {
    ...
    save(simresults500, file = paste0("mainSimResults/","simsCATE", hard,pos, "n500_xgboost_2"))
    ...
    save(simresults1000, file = paste0("mainSimResults/","simsCATE", hard,pos, "n1000_xgboost_2"))
    ...
    save(simresults2500, file = paste0("mainSimResults/","simsCATE", hard,pos, "n2500_xgboost_2"))
    ...
    save(simresults5000, file = paste0("mainSimResults/", "simsCATE", hard,pos, "n5000_xgboost_2"))
    ...
    save(simresults10000, file = paste0("mainSimResults/","simsCATE", hard,pos, "n10000_xgboost_2"))
  }}
