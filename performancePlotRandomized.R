...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      load(paste0("mainSimResults/simsCATE_Randomized", hard, pos,  "n", n, "_3"))
    ...
    dt <- dt[!(dt$lrnr %in% c("xgboost_3", "ranger_13")),]
    ...
    if(use_oracle_sieve){
      ggsave(paste0("mainSimResults/performancePlotRandomizedTrees_oracleSieve", "pos=",pos, "hard=",hard, ".pdf"))
    } else {
      ggsave(paste0("mainSimResults/performancePlotRandomizedTrees", "pos=",pos, "hard=",hard, ".pdf"))
    }
    ...
    plt <- ggplot(dt_tmp, aes(x = n, y = risks_best, group = type, color = type, linetype = type)) + geom_line() +
      facet_wrap(~lrnr, scales = "free") + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) + ylab("MSE") + scale_y_log10(limits = range(dt_tmp$risks_best))+  scale_x_log10(breaks = c(500, 1000, 2500, 5000, 10000))
    if(use_oracle_sieve){
      ggsave(paste0("mainSimResults/performancePlotRandomizedGAMS_oracleSieve", "pos=",pos, "hard=",hard, ".pdf"))
    } else {
      ggsave(paste0("mainSimResults/performanceRandomizedPlotGAMS", "pos=",pos, "hard=",hard, ".pdf"))
    }
    plt
})
  }
