...
for(pos in pos_list){
  for(hard in hard_list) {
    ...
    sims_list <- lapply(ns, function(n) {
      load(paste0("mainSimResults/simsCATE", hard, pos,  "n", n, "_3"))
    ...
    if(use_oracle_sieve){
      ggsave(paste0("mainSimResults/performancePlotTrees_oracleSieve", "pos=",pos, "hard=",hard, ".pdf"))
    } else {
      ggsave(paste0("mainSimResults/performancePlotTrees", "pos=",pos, "hard=",hard, ".pdf"))
    }
